...
| 406 | Legacy World Cup **410 Gone** | 어디서나 | php | 🟢 활성 | ⭐ 축구 URL 제거. **우선순위 1 필수** |
...
### 406 — Legacy World Cup 410 Gone ⭐

**2026-08-14 301 → 410 교체. 2026-08-22 훅 우선순위 1 지정.**
...
add_action('template_redirect', function() {
    ...
    if (!$gone && preg_match('#^/insights(/.*)?$#', $path)) {
        $gone = true;
    }
    if (!$gone) {
        return;
    ...
    echo '<title>410 Gone</title></head><body>';
    echo '<h1>410 Gone</h1><p>This page has been permanently removed.</p>';
    echo '<p><a href="' . esc_url(home_url('/')) . '">Faircast</a></p>';
    echo '</body></html>';
    exit;
}, 1);
```
...
`}, 1);` 의 `1` 은 `template_redirect` 훅의 **우선순위**다. 이게 없으면(기본 10) `/insights` 맨 경로가 410을 못 받는다.

**경위 (2026-08-22 실측)**

`/insights/group-e-preview` 는 410 인데 `/insights` 와 `/insights/` 만 홈으로 200 이 나왔다. 정규식은 정상이었고, **거기까지 도달을 못 하고 있었다.**

`/insights` 는 **Insights 카테고리 슬러그와 충돌한다.** 워드프레스 코어가 `/insights` 를 `/category/insights/` 로 정규화하고, 그걸 **494(Category→Page 301)** 가 홈(`insights-landing`, post 269)으로 넘긴다. 코어와 494 모두 같은 `template_redirect` 훅에 **먼저 등록**돼 있어 406보다 앞서 실행됐다.

`/tournament` `/match` 등은 실재 카테고리가 아니라 정규화가 안 걸리고, 그래서 406까지 도달해 410이 나왔다.

**교훈: 훅 하나에 여러 리디렉션이 붙으면 정규식이 맞아도 차례가 안 온다.**

**검증 (2026-08-22)** — 14개 경로 전수

| 경로 | 결과 |
|---|---|
| `/insights` `/insights/` `/insights/group-e-preview` | 410 |
| `/tournament` `/simulate` `/match/croatia-vs-ghana` | 410 |
| `/category/insights/hello-korea/` | 200 → `/hello-korea-page/` (유지) |
| `/category/insights/hello-world/` | 200 → `/hello-world-page/` (유지) |
| `/category/port-guide/` | 200 (noindex, 유지) |
| `/hello-korea-page/` `/hello-world-page/` `/port-guide/` `/about/` `/` | 200 (유지) |
