...
⚠️ **2026-08-23 사고 — 비공개 글 7장 누락.**
자산 수집(`audit-cloudinary.ps1`)은 REST 로 **발행글만** 긁었는데,
DB URL 치환 스니펫은 `wp_posts` **전체**(초안·비공개 포함)를 돌았다.
범위가 어긋난 결과 **URL 은 jsDelivr 로 바뀌었는데 파일은 올라간 적이 없는** 이미지가 7장 생겼고,
그 글들이 8/23 공개되면서 404 로 드러났다. 원본이 Cloudinary 에 남아 있어 복구했다(태그 v4).

**→ 자산 수집과 URL 치환의 대상 범위는 반드시 같아야 한다.**
...
발행 후, 그리고 새 태그를 팔 때마다 돌린다. 브라우저 콘솔에서 실행한다
(컨테이너·PowerShell 에서는 jsDelivr 가 막힌다).

1. 글·페이지 전체에서 jsDelivr URL 을 모은다 — 본문 `<img src>` + FIFU 메타 + 페이지 본문
2. 각 URL 을 `fetch` 로 상태 확인. **200 이 아닌 것만** 추린다
3. 404 가 나오면 원본을 Cloudinary 에서 찾는다 — `https://res.cloudinary.com/dzatgu3y7/image/upload/<stem>.webp` (버전 번호 없이 접근 가능)
4. 새 파일명 규칙으로 내려받아 `faircast-images/img/` 에 넣고 **새 태그**를 판다
5. 글 본문 URL 과 FIFU 를 새 태그·새 이름으로 교체. **`featured_media` 도 0 으로 비운다**

⚠️ **태그는 불변이다.** 파일을 나중에 올려도 `@v1` 을 가리키는 URL 은 영원히 404 다.
반드시 글의 태그 부분까지 바꿔야 한다.

⚠️ jsDelivr 전파에 수십 초가 걸린다. 푸시 직후 404 가 나와도 잠시 뒤 다시 확인한다.
