# Cursor Order — Port Guide count fix + dead CSS removal (2026-07-26, order 2)

## Goal

Two small corrections to the port guide page. Same two files as order 1.

1. **Fix the 98 vs 99 mismatch.** The static directory says "98곳" while the JS directory
   button on the same page says "총 99개 기관". The difference is 해양수산부 본부, which the
   static generator omitted. Adding it makes both read 99.
2. **Remove dead CSS.** The `.fct-pg-off` rules are left over from the hand-written agency
   lists that order 1 deleted. No element uses them any more.

Files changed: `pages/port-guide/build.js`, `pages/port-guide/template.html`.
**Do not touch `data.js`, `port-atlas.js`, `port-atlas-header.js`, `loader.js`, `config.json`.**

Measured effect (verified by dry run):
- agency entries: 98 → 99
- external links: 96 → 97
- static body text: 8,517 → 8,684 chars
- template size: 8,100 → 7,512 bytes (dead CSS gone)
...
Change vs order 1: `buildDirectoryHtml()` now builds a `mofItem` adapter and prepends a
해양수산부 group. The adapter is needed because `ATLAS.MOF` uses `headquarters` and
`callCenter` where every other record uses `address` and `tel`.
...
Change vs order 1: the `.fct-pg-off` rules and their two media-query entries are removed.
Nothing else changed. All 14 `{{placeholder}}` tokens are unchanged.
...
```
findstr /C:"fct-pg-off" pages\port-guide\template.html pages\port-guide\build.js
```

Expected: **no matches**.

```
findstr /C:"mofItem" pages\port-guide\build.js
```
...
## Step 4 — Commit and push FIRST, then build

⚠️ Same rule as order 1. The build pins the CDN URL to git HEAD, so the commit must exist
and be pushed before building.

Run each line separately. PowerShell does not support `&&`.

```
git status
git add pages/port-guide/build.js pages/port-guide/template.html
git commit -m "port-guide: include MOF in directory (99), drop unused fct-pg-off CSS"
git push
```
...
4. Resolving version pin and data stamp...
   loaderUrl pinned to <new 7-char hash>
   data.js rev 31c177d5 (2026-07-26)
  Article cards: 10
  Agency entries: 99
  data rev: 31c177d5
```

- `Agencies:` must be **99**, not 98.
- `data.js rev` must still be **31c177d5** — data.js is not being changed in this order.
  If it differs, data.js was modified by mistake. Stop and report.
...
```
findstr /C:"기관 99곳" pages\port-guide\port-guide.html
```

Expected: found.

```
findstr /C:"해양수산부" pages\port-guide\port-guide.html
```

Expected: found (new group heading).
...
Report the console output from Step 5 and `git log -1 --oneline`.
Do NOT paste the generated HTML.

After this, paste `pages/port-guide/port-guide.html` into WordPress page 522
(code editor, select all, delete, paste, update). No Search Console re-request needed —
the change is too small to matter and today's request already covers it.

---

## Do NOT

- Do not edit `data.js`. The 울산·제주 coordinate bug is a separate order.
- Do not commit `config.local.json` or `port-guide.html`.
- Do not build before pushing.
